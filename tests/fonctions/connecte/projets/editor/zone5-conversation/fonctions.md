...
## `2-5-2-7-16` — [modification] Clic droit sur une sélection (Code ou Édition) → "Envoyer au prompt"
...
- **[modification] Résultat — "Remplacer" et "Copier" (au lieu de "Copier ici"/"Copier vers...")** : sur les messages issus de `sendContextChat()` (texte original connu, `hasContextReplace(msg)` = `!!msg.contextReplace`), les 2 boutons habituels sont remplacés par :
  ...
  - **"Copier"** (`copyResultToClipboard()`) : mémorise le texte dans `ProjetEditorZoneComponent.clipboardText` (mémoire de session, pas le presse-papiers OS) — affiche un retour visuel bref ("Copié !", 1,5 s). Ne cible aucune section : l'utilisateur clique ensuite où il veut dans l'éditeur (Code ou Édition) et fait **clic droit → "Coller"** (menu contextuel partagé avec "Envoyer au prompt", cf. `2-5-2-4-59`/`2-5-2-5-24`) pour l'insérer à l'endroit exact du curseur.
