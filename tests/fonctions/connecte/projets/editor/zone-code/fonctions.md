...
- **Effacer l'historique** : dans la bannière "Reprendre", un bouton icône poubelle (à gauche de "Reprendre") demande confirmation inline (Confirmer/Annuler, même pattern que la suppression d'un board) puis appelle `deleteHistory()` → `MegaOutilsService.deleteChatHistory(instanceId)` → `DELETE /api/mega-outils/prompt/:instanceId/chat-sessions` (supprime toutes les sessions **et** tous les messages de cette instance côté serveur, ces tables n'ayant pas de contrainte FK cascade). La bannière disparaît immédiatement après suppression (`resumableSession.set(null)`).
...
- **À vérifier** : créer un Prompt en mode tchat, démarrer, envoyer 2-3 messages avec markdown/formulaire/fences de test, vérifier le rendu riche, la matérialisation des MO, le round-trip formulaire, la persistance (fermer/rouvrir → bannière Reprendre → conversation identique). Changer le mode d'un Prompt existant via le sélecteur du board (Normal ↔ Guidé ↔ Tchat) → la ligne `MODE:` du fence est mise à jour en conséquence, sans toucher au reste du bloc. Cliquer le bouton poubelle de la bannière "Reprendre" → Confirmer → la bannière disparaît ; rouvrir le popup → plus aucune conversation à reprendre (vérifier aussi en BDD que `mega_outil_prompt_chat_sessions`/`_messages` sont bien vidées pour cette instance).
- **Composants:** `prompt-chat-popup.component.ts`, `prompt-board.component.ts`, `prompt-workflow-popup.component.ts`, `form-execution-popup.component.ts`, `mo-fence-parser.util.ts`, `prompt-admin.component.ts`, `mega-outils.service.ts`, `projet-editor-zone.component.ts`, `projet-editor-zone.component.html`, `server/server-data.js`
