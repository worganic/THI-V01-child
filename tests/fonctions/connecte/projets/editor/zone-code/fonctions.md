...
## `2-5-2-4-52` — Mode tchat : conversation libre + import vers l'édition
...
- **Résultat attendu** : le popup affiche IA/modèle + le premier message (prompt de la section), avec écran de saisie des variables `{{...}}` si le prompt en contient. Au clic "Démarrer le tchat", le premier message est envoyé via `AiExecuteService.startExecution()` (même mécanisme que les 2 autres modes — pas de session CLI persistante : chaque tour relance `claude`/`agy` à froid avec tout l'historique reconstruit en texte, `PromptChatPopupComponent.buildTranscriptText()`) et la réponse streame dans la conversation. L'utilisateur peut ensuite répondre librement via le composer (textarea + "Envoyer", Entrée = envoyer / Maj+Entrée = retour à la ligne) : chaque nouveau tour renvoie l'intégralité de l'historique, donc l'IA garde le contexte des échanges précédents. Aucune insertion automatique dans le document.
- **Bouton "Copier la dernière réponse → Coller en édition"** : actif dès qu'il y a au moins un message IA. Émet `insertAsSection` avec le texte de la dernière réponse IA → `onChatInsertRequested()` (dans `projet-editor-zone.component.ts`) ouvre le **popup d'import existant** (`pastePreview`, cf. `2-5-2-4-44`) ciblé sur le dossier du Prompt, au niveau juste en dessous (même convention que `PR-Res`). Le popup tchat **ne se ferme pas** : après avoir validé ou annulé l'import, l'utilisateur retrouve la conversation intacte et peut continuer à discuter.
- **À vérifier** : créer un Prompt en mode tchat, démarrer, envoyer 2-3 messages, vérifier que l'IA répond en tenant compte des tours précédents. Cliquer le bouton de copie → le popup d'import s'ouvre avec le texte de la dernière réponse et le bon niveau de titre proposé → "Coller" → le texte apparaît dans le document sous la bonne section → le popup tchat est toujours ouvert et utilisable derrière. Changer le mode d'un Prompt existant via le sélecteur du board (Normal ↔ Guidé ↔ Tchat) → la ligne `MODE:` du fence est mise à jour en conséquence, sans toucher au reste du bloc.
- **Composants:** `prompt-chat-popup.component.ts`, `prompt-board.component.ts`, `projet-editor-zone.component.ts`, `projet-editor-zone.component.html`
