...
- **[modification] Exécution — plus de popup** : le bouton ▶ Exécuter ne déclenche plus de popup (les 4 anciens composants popup ont été supprimés) mais bascule l'onglet Conversation (zone 5) et y lance la conversation, avec l'IA/modèle déjà sélectionnés dans cet onglet — voir le détail complet en `2-5-2-7-12`.
- Variables `{{x}}` : si le prompt contient des variables, une carte en ligne (pas un popup) dans l'onglet Conversation affiche un formulaire de substitution avant l'envoi
- Streaming : exécution via `GET /api/mega-outils/prompt/execute-stream` (EventSource SSE, `AiExecuteService`, inchangé) ; tous les providers passent par l'executor local port 3002 ; AGY utilise un fichier de sortie pollé toutes les 1500ms (agy ne streame pas sur stdout Windows)
- Insertion : le bouton "Copier vers l'édition" (sur chaque message IA de la conversation) ouvre le popup d'import existant (`pastePreview`) ; le livrable matérialisé (mode Guidé/Tchat) est placé dans une section « PR-Res NomPrompt » via `upsertPromptResultSection` (inchangé)
...
- **[modification] Lancement — plus de popup** : bouton « Exécuter » d'un prompt guidé bascule l'onglet Conversation et y lance le cadrage (voir `2-5-2-7-12`) ; charge les 3 prompts globaux (base + cadrage + génération) via `getPromptGlobalConfig`, comme avant
- Phase cadrage : l'IA reçoit le méta-prompt de cadrage (system) + la demande (user) et répond par un formulaire Markdown ; parsé en questions, affiché en ligne dans la conversation via `app-form-execution-popup [inline]="true"` (bouton secondaire « Générer le livrable maintenant »)
