...
export class DocumentsComponent implements OnInit {
  ...
  async loadCategories() {
    ...
    try {
      const res = await fetch(`${this.apiUrl}/api/doc-categories`, { headers: this.authHeaders });
  ...
  async saveCategory() {
    ...
    try {
      if (editing) {
        ...
        const res = await fetch(`${this.apiUrl}/api/doc-categories/${editing.id}`, {
          method: 'PUT',
        ...
        const res = await fetch(`${this.apiUrl}/api/doc-categories`, {
          method: 'POST',
          headers: this.authHeaders,
          body: JSON.stringify({ name: this.catName, description: this.catDescription })
        });
  ...
  async deleteCategory(id: string) {
    ...
    try {
      const res = await fetch(`${this.apiUrl}/api/doc-categories/${id}`, {
        method: 'DELETE',
        headers: this.authHeaders
      });
      if (!res.ok) throw new Error((await res.json()).error || 'Erreur');
  ...
  async saveDefaultDocument(catId: string) {
    ...
    this.categoriesError.set('');
    try {
      const res = await fetch(`${this.apiUrl}/api/doc-categories/${catId}/default-document`, {
        method: 'PUT',
        headers: this.authHeaders,
  ...
  async toggleDefaultDoc(doc: DocDocument) {
    ...
    try {
      const res = await fetch(`${this.apiUrl}/api/doc-categories/${doc.categoryId}/default-document`, {
        method: 'PUT',
        headers: this.authHeaders,
  ...
  async loadDocuments() {
    ...
    this.documentsError.set('');
    try {
      const res = await fetch(`${this.apiUrl}/api/documents`, { headers: this.authHeaders });
      if (!res.ok) throw new Error((await res.json()).error || 'Erreur');
  ...
  async saveDocument() {
    ...
    try {
      ...
      if (editing) {
        ...
        const res = await fetch(`${this.apiUrl}/api/documents/${editing.id}`, {
          method: 'PUT',
          headers: this.authHeaders,
  ...
  async deleteDocument(id: string) {
    ...
    try {
      const res = await fetch(`${this.apiUrl}/api/documents/${id}`, {
        method: 'DELETE',
        headers: this.authHeaders
      });
      if (!res.ok) throw new Error((await res.json()).error || 'Erreur');
