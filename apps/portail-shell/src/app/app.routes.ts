...
import { authGuard, guestGuard } from '@portail/core-auth';
...
export const appRoutes: Routes = [
  {
    path: '',
    pathMatch: 'full',
    // './landing/landing.slot' (pas './landing/landing.component' directement) :
    // fichier généré par tools/generate-page-overrides.mjs, régénéré avant
    // chaque build/serve. Voir docs/architecture-sous-applications.md, section
    // "Surcharge de page par présence de fichier".
    loadComponent: () => import('./landing/landing.slot').then(m => m.LandingComponent),
  ...
  {
    path: 'home',
    // './home/home.slot' (pas './home/home.component' directement) : fichier
    // généré par tools/generate-page-overrides.mjs, régénéré avant chaque
    // build/serve. Voir docs/architecture-sous-applications.md, section
    // "Surcharge de page par présence de fichier".
    loadComponent: () => import('./home/home.slot').then(m => m.HomeComponent),
  ...
  },
  {
    ...
    children: [
      ...
      { path: 'config/design', loadComponent: () => import('./admin/admin-design/admin-design.component').then(m => m.AdminDesignComponent) },
      // Route générique : rend l'admin qu'une sous-application a enregistrée via
      // AdminTabsRegistryService (voir admin-app-host.component.ts) — une seule
      // route ici plutôt qu'une par sous-application.
      { path: 'app/:id', loadComponent: () => import('./admin/admin-app-host/admin-app-host.component').then(m => m.AdminAppHostComponent) },
